...
@Entity
public class ClickySearchesNewest extends Model {
	...
	public static Finder<Long,ClickySearchesNewest> find = new Finder<>(Long.class,ClickySearchesNewest.class);
	
	public static List<ClickySearchesNewest> getAll(Date sDate,Date eDate) {
		return find.where().between("saveDate", sDate, eDate).findList();
	}
	
	
	public static List<ClickySearchesNewest> getAllData(Date sDate) {
		return find.where().eq("saveDate", sDate).findList();
	}
	
	
	public static List<ClickySearchesNewest> getfindAll() {
		return find.all();
	}
	
	
	public static ClickySearchesNewest findById(Long id) {
		return find.byId(id);
	}
	
	
	public static List<ClickySearchesNewest> findByTitleAndDate(String title, Date startdate, Date enddate) {
